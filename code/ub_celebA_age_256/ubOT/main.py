...
# load trained model
model = AENet.VAE(nc=3, ngf=128, ndf=128, latent_variable_size=args.nz, batchnorm=False)
...
#================= Log results ===========================================
    ...
    # visualize progress
    im_inputs = model.generate(s_inputs)
    im_trans = model.generate(s_generated)
    im_targets = model.generate(t_inputs)

    if epoch % 20 == 0:
        tracker_plot, scale_plot = utils.plot(tracker, tracker_plot, scale_plot, im_inputs.cpu().data.numpy()[0:16], im_trans.cpu().data.numpy()[0:16], s_scale.cpu().data.numpy(), im_targets.cpu().data.numpy()[0:16], args.env, vis)
