...
class CombinedDataset(Dataset):
    ...
    def combine_data(self):
        ...
        for i in range(10):
            data_temp = train_data[train_label==i]
            brightness = np.array([np.sum(i) for i in data_temp])
            sort_idx = np.argsort(brightness)
            traindata_classes.append(data_temp[sort_idx])
            trainlabels_classes.append(train_label[train_label==i])
    ...
    def __getitem__(self, idx):
        img1, lab1 = self.data1[idx], self.labels1[idx]
        img1 = Image.fromarray(img1, mode='L')
        img1 = self.transform(img1)
        return (img1.view(-1), lab1)
