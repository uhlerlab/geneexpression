...
#            G_loss = torch.mean(s_scale*torch.sum(mse(s_generated, s_inputs), dim=1)) - args.lamb1*torch.mean(s_scale*(1-torch.log(s_scale)))
            ...
            if args.psi2 == "EQ":
                ...
                G_loss += args.lamb2*torch.mean((LOG2.expand_as(s_outputs)+logsigmoid(s_outputs)-s_outputs))
            ...
            netGS.zero_grad()
            GS_loss = args.lamb0*torch.mean(s_scale*torch.sum(mse(s_generated, s_inputs), dim=1)) + args.lamb1*torch.mean((s_scale-1)*torch.log(s_scale))
            GS_loss += calc_gradient_penalty_rho(netGS, s_inputs.data, s_inputs.data[torch.randperm(s_inputs.size(0))])
            GS_loss += args.lamb2*torch.mean(s_scale*(LOG2.expand_as(s_outputs)+logsigmoid(s_outputs)-s_outputs))
...
#================= Log results ===========================================
    ...
    torch.save(netGS.cpu().state_dict(), args.save_name+"_netGS.pth")
